...
#   Terraform  = the brain (reads your files, builds the plan)
#   Provider   = the hands (knows the Azure API, makes the actual calls)
#
# One more mental model that makes everything click: Terraform is
# DECLARATIVE. You never write "create a VNet, then add a subnet" — you
# write "a VNet and a subnet EXIST", and Terraform works out the steps
