...
3. **Deny test #1 (the key result)**, incognito, sign in as the junior dev, try to
...
9. **Deny test #2 (the key result)**, as the Owner, try to create `vm-policy-test`
   at `Standard_D2s_v3` → **Validation failed · Policy check failed ·
   Restrict-VM-Sizes**. Policy governs *what*, even for admins.

> **Policy propagation:** an assignment can take **15 to 30 min** to fully enforce.
> On the shoot we assign in step 5, then do budget → lock → secure-score (steps
> 6 to 8, all quick and unrelated) to burn that clock, and run the deny test last.
