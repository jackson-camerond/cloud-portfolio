...
Modernizing to PaaS & Securing Secrets: the running Lab 02 web tier stays up while its
...
Two resource groups, linked only logically. `rg-lab02-cam` keeps the VNet, `vm-web-01`,
and Bastion running unchanged; the decommissioned `vm-db-01` is shown struck through.
`rg-lab03-cam` is new, has no VNet, and holds `sql-lab03-cam`/`sqldb-app` behind a
firewalled public endpoint and `kv-lab03-cam`. The diagram separates the two runtime
flows: the request path (browser → `vm-web-01` → TLS/TDS → `sqldb-app`) and the
managed-identity secret-fetch path (`vm-web-01` → IMDS → Key Vault, RBAC-checked).

## How it runs

![Flowchart: links.service running its ExecStartPre fetch script through IMDS, a Key Vault RBAC check, and a tmpfs-only secret file before the app starts](diagram/flowchart.png)

This is the secret-fetch path only, scoped narrowly because it's the lab's actual
subject. Every time `links.service` starts, `ExecStartPre` calls IMDS for a bearer
token, exchanges it with Key Vault for `DbAppPassword` (denied with a 403 if the RBAC
role isn't assigned), and writes the result to a tmpfs file that only exists in RAM.
systemd then stacks that file on top of `/etc/app.env` so the app process gets
`DB_PASS` in its environment without it ever touching disk, and a restart or reboot
wipes the file until the next start runs the fetch again.
...
- `app.env.lab03` - the new `/etc/app.env`: Azure SQL host + `sqldb-app` + `appuser` +
  `TDSVER=7.4`, and **no password line**. This file IS the money shot.
...
- **403 from Key Vault:** RBAC propagation takes 1-5 min after the role assignment;
